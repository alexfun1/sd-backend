...
def txt2img_tab():
    ...
    with txt2img:
        with gr.Row():
            with gr.Column():
                with gr.Row():            
                    models = gr.Dropdown(label="Select Model", choices=["pocsd15", "pocsdxl","pocpony"], value="pocsd15")
                    img_gen_qty = gr.Slider(label="Number of Images", minimum=1, maximum=4, value=1, step=1)
                with gr.Row():
                    with gr.Column():
                        width = gr.Slider(label="Width", minimum=512, maximum=1024, value=768, step=64)
...
def txt2img_chat_tab():
    ...
    with txt2img_chat:
        ...
        with gr.Sidebar(open=False, position="right"):
            models = gr.Dropdown(label="Select Model", choices=h.get_models_list())
            width = gr.Slider(label="Width", minimum=512, maximum=1024, value=768, step=64)
            height = gr.Slider(label="Height", minimum=512, maximum=1024, value=512, step=64)
            orientation = gr.Radio(label="Orientation", choices=["Landscape", "Portrait"], value="Landscape")
            seed = gr.Number(label="Seed", value=-1, precision=0)
        
        def send_message(message, chat_history, models, width, height, seed):
            response = di.txt2img(message,"", models,width, height,seed)
        ...
        msg.submit(send_message, [msg, chatbot,models,width,height, seed], [msg, chatbot])
    return txt2img_chat